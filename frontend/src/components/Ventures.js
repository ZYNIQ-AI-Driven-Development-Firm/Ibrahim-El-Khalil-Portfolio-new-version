...
const Ventures = () => {
  ...
  const [venturesData, setVenturesData] = useState(VENTURES_DATA);
  const [whitePapersData, setWhitePapersData] = useState(WHITE_PAPERS_DATA);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadData = async () => {
      try {
        const [ventures, papers] = await Promise.all([
          API.getVentures(),
          API.getWhitePapers()
        ]);
  ...
  const nextSlide = () => {
    setCurrentSlide((prev) => (prev + 1) % venturesData.length);
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + venturesData.length) % venturesData.length);
  };
  ...
  const nextPaperSlide = () => {
    setCurrentPaperSlide((prev) => (prev + 1) % whitePapersData.length);
  };

  const prevPaperSlide = () => {
    setCurrentPaperSlide((prev) => (prev - 1 + whitePapersData.length) % whitePapersData.length);
  };
        ...
        <div className="relative overflow-hidden">
                  ...
                  <div className="relative z-10 space-y-4">
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                      <div className="lg:col-span-3">
                        ...
                        <h4 className="text-lg text-primary-400 mb-2">{venture.role}</h4>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {venture.technologies.map((tech, i) => (
                              <span 
                                key={i} 
                                className="px-2 py-1 bg-primary-500/20 text-primary-300 rounded-md text-xs border border-primary-500/30"
                              >
                                {tech}
                              </span>
                            ))}
          ...
          <div className="flex justify-center mt-4">
            <div className="text-sm text-gray-400">
              {currentSlide + 1} of {venturesData.length}
            </div>
          </div>
        </div>
